...
class api_cor_res(correct):
    def res_results(self, cookie, api_version):
        """
        发送正确参数的请求
        ...
        for i in range(len(self.api_url())):
            results = requests.post("http://192.168.31.99:8088/" + api_url[i], data={"APIVersion": api_version},
                                    params=api_param[i], cookies=cookie)
            response_result.append(results)
        return response_result


class api_err_res(error):
    def res_results(self, cookie, api_version, error_list):
        """
        发送带有错误参数的请求
        :param cookie: value参数为cookie的值
        :param api_version: apiversion
        ...
        for i in range(len(p_e[0])):
            ...
            for j in range(len(p_e[0][i])):
                ...
                for k in range(len(error_list)):
                    results = requests.post("http://192.168.31.99:8088/" + api_url[i], data={"APIVersion": api_version},
                                            params=dict(zip(_KEY[k], _VALUE[k])), cookies=cookie)
